...
const Card = ({ card, showProduct, renderIcon, navigate }) => {
  return (
    <div
      className="border border-white/30 hover:shadow-lg hover:shadow-white/60 bg-black/40 cursor-pointer w-full h-auto rounded-lg overflow-hidden transform transition-all duration-300"
    ...
    >
      ...
      <div className="px-4 py-3 text-center">
        <p className="text-white text-2xl font-semibold truncate">{card.title}</p>
        
      </div>
      <div className="flex justify-center pb-4">
        <button
          className="w-10/12 md:w-4/5 lg:w-3/4 py-2 border border-black text-sm font-medium rounded-md bg-white text-black hover:bg-black/50 hover:text-white transition-all duration-200"
          onClick={(e) => {
            e.stopPropagation();
